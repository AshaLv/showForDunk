...
<body>	
	...
	<%
	//把鞋子的id参数取到
	String shoeId = request.getParameter("nikeShoeId");
	String shoe = "nike" + shoeId +".jpg";
	%>
	<!--放置鞋子图片和表单-->
	...
	<div class="sectionForComment">
		<div class="container">
			<div class="row">
				<div class="col-lg-5"><img src="../img/<%=shoe%>" alt="the shoe " class="commentImg" width="200" height="200"></div>
			...
			<div class="row">
				...
				<div class="col-lg-2"></div>
				<br><br><br><br><br>
				<div class="col-lg-5"><input type="hidden" value="<%=shoeId%>" name="nikeShoeId"><input type="submit" class="commentButton" name="commentSubmit" value="评论"></form>
				</div>
			</div>
			<div class="row">
	...
	String root = request.getRealPath("/") + "shoeForDunk" + "\\nikeShop" + "\\commentNike";//取得目前目录在的位置
	String wholeRoot = "\\commentFile\\"+shoeId+".txt";
	FileWriter commentNikeWriteFw = new FileWriter(root + "\\commentFile\\"+shoeId+".txt",true);//建立FileWrite物件，以Append()方式写入资料
	BufferedWriter commentNikeWriteBw = new BufferedWriter(commentNikeWriteFw);//建立bufferedWriter物件
	String commentNikeFinal = "姓名:<font size=4 color= brown>" + loginNameCookie + "</font><br>留言时间：" + 
		timeNow + "</font><br>留言：" +
		"<font size = 4 color=red>" +
		request.getParameter("message") + "</font><hr >";//要记录到txt的资料
	commentNikeWriteBw.write(commentNikeFinal);//完成记录

	commentNikeWriteBw.close();//关闭bw物件
	commentNikeWriteFw.close();//关闭fw物件
